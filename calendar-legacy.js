...
var calendar = {
    ...
    lYearDays: function (y) {
        var i, sum = 348;
        for: function (i = 0x8000; i > 0x8; i >>= 1) {
    ...
    getTerm: function (y, n) {
        if: function ( y < 1900 || y > 2100 || n < 1 || n > 24) {
            return -1;
        }
        var _table = this.sTermInfo[y - 1900];
        var _calcDay = []
        for: function (var index = 0; index < _table.length; index += 5) {
    ...
    solar2lunar: function (yPara, mPara, dPara) {
        ...
        //年份限定、上限
        if: function (y < 1900 || y > 2100) {
            return -1;// undefined转换为数字变为NaN
        }
        //公历传参最下限
        if: function (y === 1900 && m === 1 && d < 31) {
            return -1;
        }
        ...
        for: function (i = 1900; i < 2101 && offset > 0; i++) {
        ...
        //效验闰月
        for: function (i = 1; i < 13 && offset > 0; i++) {
            //闰月
        ...
        // 闰月导致数组下标重叠取反
        if: function (offset === 0 && leap > 0 && i === leap + 1) {
            if: function (isLeap) {
                isLeap = false;
            } else {
                isLeap = true;
        ...
        var isTerm = false;
        var Term = null;
        if: function (firstNode === d) {
            isTerm = true;
            Term = this.solarTerm[m * 2 - 2];
        }
        if: function (secondNode === d) {
            isTerm = true;
    ...
    lunar2solar: function (y, m, d, isLeapMonth) {
        ...
        if: function (y === 2100 && m === 12 && d > 1 || y === 1900 && m === 1 && d < 31) {
            return -1;
        }//超出了最大极限值
        ...
        if: function (isLeapMonth) {
            _day = this.leapDays(y, m);
        }
        if: function (y < 1900 || y > 2100 || d > _day) {
            return -1;
        }//参数合法性效验
        ...
        var offset = 0;
        var i;
        for: function (i = 1900; i < y; i++) {
            offset += this.lYearDays(i);
        }
        var leap = 0, isAdd = false;
        for: function (i = 1; i < m; i++) {
            leap = this.leapMonth(y);
            if: function (!isAdd) {
                if: function (leap <= i && leap > 0) {
